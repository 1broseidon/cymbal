...
**`impls` follows the same rules.** On a type nothing implements, or with `--of` on a type that implements nothing, `impls` used to print a note on stderr and exit 0, the same as for a name that doesn't exist, and printed no JSON with `--json`. Now it prints its usual header with `implementor_count: 0` or `edges: 0`, or an empty `results` list, and a name the index has never seen exits 1 with `symbol not found: X`, in text, `--json` and graph output. An interface from outside the repo that a type embeds or implements, like `io.Reader`, is still known, by its bare name: `impls Reader`. With several names, `--json` still returns a map keyed by name, and a missing name keeps its key, with the error. A query error now fails the command instead of being printed with exit 0. A script that read `impls`' exit status or its stderr note should check the count instead.

**`impls` finds more implementors in Java, Kotlin, Scala, Dart and PHP.** A generic or qualified supertype is now recorded under its bare name, as in other languages, so `impls Base` finds `class H extends Base<String>` and `class I extends a.b.Base` in Java, which used to record no edge at all. Kotlin stored `class E : kotlinx.coroutines.Job` under `kotlinx` instead of `Job`, Scala dropped a qualified mixin like `with a.b.Mix`, Dart added a stray `a` for `implements a.Iface` and dropped a `with` mixin after `extends`, and PHP stored `\A\B\Base` whole, so `impls Base` missed it. The first refresh after upgrading reparses every file once, with a one-line note on stderr.
...
**v0.16.0 and v0.16.1 were tagged but never published.** Tests that failed only on macOS and Windows stopped their release builds. 0.16.2 is the same release with those tests fixed, and CI now runs the tests on macOS and Windows too.
...
- **JS/TS/TSX arrow-function assignments and hook-wrapped bindings** — `const`/`let`/`var` arrows (`const handleSubmit = async (data) => …`) and first-argument wrappers (`useCallback`, `useMemo`, `memo`, `forwardRef`, and other bare-identifier functions) become function symbols with the wrapped function's signature, nested under their parent so `outline` / `search` / `show` / `refs` can see them. Unwrapping follows only the first argument, so trailing-callback utilities (`sortBy(items, fn)`) stay non-functions, and recurses through composed wrappers (`memo(forwardRef((props, ref) => …))`). First-argument callbacks such as `setTimeout(() => work(), ms)` and `useMemo(() => value)` also classify as functions. For a double-arrow factory such as `useMemo(() => (rows) => …)`, the reported signature is the outer thunk's `()`, not the returned function's `(rows)`. (#68, #76)
- **Namespace-qualified JS/TS/TSX hooks and wrappers** — `React.useCallback`, `MyHooks.useDebounce`, and namespace-qualified `memo` / `forwardRef` / `lazy` unwrap when the receiver is a bare identifier. Hook names must start with `use` followed by an uppercase ASCII letter. Calls such as `arr.map`, `emitter.on`, `styled.button`, `this.useThing`, chained `App.Hooks.useThing`, and computed `React["useCallback"]` remain excluded. (#76)
- **`cymbal ls --names`** — the indexed file inventory the Glob nudge has advertised since #47 but that never existed (`unknown flag: --names`). Prints a sorted, repo-relative list from the `files` table — the code inventory after skip rules (generated/vendored/large files absent). An optional pattern uses the same `--path`/`--exclude` semantics (plain substring, or glob with `**`); `--lang` filters by indexed language (names as shown by `ls --stats`); `--null` NUL-terminates for `xargs -0`; `--json` emits `results: []`, never null. The Glob nudge now passes the original pattern through (`cymbal ls --names '**/*.go'`), stays silent when the Glob call has an explicit path root (except `.`), and brace patterns (`**/*.{go,ts}`) suggest the unfiltered inventory because `pathmatch` has no brace expansion. Library: `index.ListFileNames`.

### Changed

- **BREAKING: `investigate --json` now returns one stable payload shape for any symbol count** — inside the existing `{version, results}` JSON envelope, a single-symbol payload used to be an object and a batch payload an array. Both now use `{symbols, resolve_scope, results: [...]}`, and each per-symbol entry carries its own `symbol` key. Consumers should iterate `.results.results[]` for either input mode; read a successful entry's `.result` or an unsuccessful entry's `.error`. For a single result, `.results.result` moves to `.results.results[0].result`; batch entries move from `.results[]` to `.results.results[]`. The outer envelope still reports `"version": "0.1"`, so upgrade consumers together with the CLI. (#66)
...
- **`cymbal hook remind` covers the full command set** — reorganized around tasks (orientation, reading source, following relationships, assessing change risk, finding things) so agents pick the right verb without the manual. Adds `context`, `changed`, `diff`, `structure`, `outline`, `refs`, `importers`, and qualified-name `show Parent.child`.
- **`cymbal hook nudge` uses per-tool templates for Read and Glob** — the single search-oriented template ("ranked results with file:line") was misleading when Claude's Read or Glob tools fired the hook. Read now suggests `cymbal outline <file>` (with Why pointing at `show <file>:<Symbol>` and line ranges); Glob frames `ls --names` as the indexed inventory. Each template keeps an explicit "ignore this if your original tool was right" branch (whole-file Read, exact glob / non-code files).
- **Bumped Go toolchain floor to 1.26.7** — 1.26.5 closed the `crypto/tls` Encrypted Client Hello privacy leak (GO-2026-5856) reached from the update checker; 1.26.7 covers four later stdlib findings that `govulncheck` flagged on main. CI uses `go-version-file: go.mod`; local builds with Go 1.21+ auto-fetch the new toolchain.
...
- **`show file:Symbol` no longer misroutes names that start with `L`** — `isFilePath` treated any `:L…` suffix as a line range, so `cymbal show index.go:ListRepos` or `store.go:Load` failed with "file not found". Only a numeric suffix (optional leading `L`) is a line range now. The `file:Symbol` hint — already in `flexResolve` — is documented in help as narrowing (suffix/contains match, so `store.go` also matches `mystore.go`) and falls back to global results when the hint matches nothing. Not-found errors name both the symbol and the file hint when nothing matched before `--path`/`--exclude` filtering.
- **`index --force <subdir>` wrote to an orphaned DB** — the DB path was hashed from the subdirectory instead of the git root, so the indexed data was invisible to every query command, and a scoped walk could prune files outside the subtree. Indexing now resolves the git root (via `EvalSymlinks` first, so a symlink into a subtree also lands in the right DB), walks and prunes only within the scope, and keeps `rel_path` repo-relative. A scope outside the repo root is rejected; a scope equal to the root is a full unscoped index. A first-ever scoped index persists `--exclude`/`--include-*` so the next auto-refresh completes the repo with the same flags; scoped runs over an existing DB print a note that filter flags are run-local.
