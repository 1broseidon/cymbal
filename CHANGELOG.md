...
- **JS/TS/TSX arrow-function assignments and hook-wrapped bindings are now indexed** — `const`/`let`/`var` arrows (`const handleSubmit = async (data) => …`) and first-argument wrappers (`useCallback`, `useMemo`, `memo`, `forwardRef`, and other bare-identifier HOFs) become function symbols with the inner signature, nested under their parent so `outline` / `search` / `show` / `refs` can see them. Unwrap is first-argument only, so trailing-callback utilities (`sortBy(items, fn)`) stay non-functions, and it recurses through composed wrappers (`memo(forwardRef((props, ref) => …))`). Member-expression callbacks (`arr.map`, `emitter.on`) are excluded. First-argument callbacks (`setTimeout(fn, ms)`) and `useMemo` of a plain value still classify as functions.
