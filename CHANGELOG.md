...
- **Codecov now uses the same product coverage denominator as CI** — `codecov.yml` ignores the internal bench harness and enforces an 80% project target, while command entrypoint regression tests keep Codecov's line-oriented calculation above the threshold.
